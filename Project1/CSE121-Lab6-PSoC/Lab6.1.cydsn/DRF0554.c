...
void send(uint8_t *data, uint8_t len){
    Cy_SCB_I2C_MasterSendStart(I2C_ADXL_HW,_lcdAddr,CY_SCB_I2C_WRITE_XFER,0,&I2C_ADXL_context);
    for (int i = 0; i < len; i++) {
        Cy_SCB_I2C_MasterWriteByte(I2C_ADXL_HW,data[i],0,&I2C_ADXL_context);
        CyDelay(5);
    }
    Cy_SCB_I2C_MasterSendStop(I2C_ADXL_HW,0,&I2C_ADXL_context);  
}
...
void setReg(uint8_t addr, uint8_t data){
    Cy_SCB_I2C_MasterSendStart(I2C_ADXL_HW,_RGBAddr,CY_SCB_I2C_WRITE_XFER,0,&I2C_ADXL_context);
    Cy_SCB_I2C_MasterWriteByte(I2C_ADXL_HW,addr,0,&I2C_ADXL_context);
    Cy_SCB_I2C_MasterWriteByte(I2C_ADXL_HW,data,0,&I2C_ADXL_context);
    Cy_SCB_I2C_MasterSendStop(I2C_ADXL_HW,0,&I2C_ADXL_context);  
}
...
bool LCD_Start(CySCB_Type *master, cy_stc_scb_i2c_context_t *context){
   ...
   cy_en_scb_i2c_status_t init_status;
    ...
    init_status = Cy_SCB_I2C_Init(master, &I2C_ADXL_config, context);
    sys_status = Cy_SysInt_Init(&I2C_ADXL_SCB_IRQ_cfg, &I2C_ADXL_Interrupt);
